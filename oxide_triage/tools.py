...
AGENT_RULES = (
    ...
    "user in plain words and then stop: in this app they answer with a button beside your reply, "
    "not in the conversation. The confirmed flag is ignored on anything you send, so calling the "
    "tool again with confirmed=true only holds the run a second time, and their 'yes' in a message "
    "does not authorise it however plainly it is worded. Do not promise to run it next; say it is "
    "waiting on them. `parse_request` only previews, and the questions it returns carry no button "
    "with them: when the user wants the run, call `triage` or `rerun` and let the hold raise the "
    "confirmation. Keep replies short and concrete; the full report is shown beside the chat, so "
