...
Production caveats are deterministic rules only. The legacy selection helper can return
existing caveats, never model prose or new evidence. Free-form conversation is interpretation,
not semantically verified evidence.
"""
...
def rule_caveats(sc: ScoredCandidate, eff: Effective, config: Config) -> list[Caveat]:
    ...
    if (
        ...
        add(
            ...
            f"{(iface.x_substrate or 0):.0%} {iface.substrate}, forming {prods}. This is a bulk driving force, "
            f"not a prediction of film compatibility; kinetics and epitaxy are not modelled (hull: {iface.thermo_type}).",
...
REFUTE_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        "caveat_codes": {"type": "array", "maxItems": 3, "items": {"type": "string"}},
    },
    "required": ["caveat_codes"],
}

REFUTE_SYSTEM = (
    "Select up to three existing rule_caveats codes worth emphasizing for this candidate. "
    "Return only caveat_codes. Do not write assertions, values, citations, or new caveats."
)
...
def numeric_guard(text: str, facts: dict[str, Any]) -> bool:
    """Fail-closed check for canonical statements from structured, attributed facts.

    Arbitrary prose is not semantically validated. Flat numeric bags are deliberately
    rejected; callers must supply a ``statements`` list generated by evidence.statements.
    """
    from oxide_triage.evidence import GroundedFact

    statements = facts.get("statements", [])
    return any(isinstance(f, GroundedFact) and text == f.render() for f in statements)
...
def candidate_facts(sc: ScoredCandidate) -> dict[str, Any]:
    ...
    return {
        ...
        "stability": r.stability.model_dump(),
        "cross_check_oqmd": r.cross_check.model_dump(),
        "cross_source_agreement": sc.cross_source_agreement,
        "band_gap": sc.band_gap_assessment.model_dump(),
        "figure_of_merit": r.figure_of_merit.model_dump(),
        "hazard": r.hazard.model_dump(),
        "literature": r.literature.model_dump(),
        "score": {
            "adjusted": sc.adjusted_score,
            "raw": sc.raw_score,
            "data_coverage": sc.data_coverage,
            "data_coverage_label": sc.confidence,
        },
...
def llm_caveats(sc: ScoredCandidate, llm: LLMClient) -> list[Caveat]:
    """Compatibility helper: select existing rules only; unused by production refutation."""
    facts = candidate_facts(sc)
    data = llm.complete_json(REFUTE_SYSTEM, wrap_retrieved(facts, "triage_facts"), REFUTE_SCHEMA)
    if not isinstance(data, dict) or set(data) != {"caveat_codes"}:
        return []
    codes = data["caveat_codes"]
    if not isinstance(codes, list):
        return []
    # The selection never creates a new caveat or changes its source or severity.
    known = {c.code: c for c in sc.caveats if c.origin == "rule"}
    return [
        known[code].model_copy(deep=True)
        for code in dict.fromkeys(c for c in codes[:3] if isinstance(c, str) and c in known)
    ]
...
def refute(
    ...
    """Attach only deterministic caveats. Legacy LLM configuration remains loadable.

    Conversation handles interpretation; spending a model call selecting already-attached
    caveats adds no evidence. Keep the parameter for existing callers/deployments.
    """
    for sc in shortlist:
