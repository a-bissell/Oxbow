...
*Deployable agentic triage for a materials-science research centre. Prototype, one week; two material classes.*

## 1. What the tool is for, and what it is not

A research centre wants its scientists to triage candidate materials before committing bench time. The brief's worked example was a PI asking for oxide dielectrics for thin-film experiments; it asks for something **useful, honest, traceable, configurable and robust**, with no wet-lab actions, private data or paywalled sources.

The tool takes a natural-language request, ranks known materials from cached public data against explicit criteria, and returns a shortlist where every number traces to a source, every gap is named, and every entry carries the arguments against it. It does not propose new materials or make publishable claims. The oxide-dielectric instance is built out in full because only a worked instance can show that the citations are real and the caveats are the ones a scientist would raise; a second instance, thermal barrier coatings, shows that the instance is not the system (section 6).
...
A model's characterisation of a deterministic result ("this looks promising") propagates downstream with the authority of the value it describes, so computation and narration never share a code path. The core (`scoring/`) imports nothing from `edges/`; the renderer receives a finished result and a template; the refutation stage may annotate structured facts but cannot touch a rank or a score. A run's `config_hash` and `cache_fingerprint` identify the answer: same query, same cache, same output, next month too. The model is optional everywhere; with one configured it may fill parser fields the rules left at default (never an element decision) and add up to three observations per candidate, each citing an existing fact field and containing no number absent from the facts. Anything else is discarded.

## 3. Data layer

Public sources only, each cached in SQLite with a retrieval timestamp. **Materials Project** supplies the candidate universe (oxygen plus one or two cations from a versioned allowlist, observed structures only), hull and band-gap data with the functional read from the producing task, the hull phases behind the interface criterion, and the property behind the figure of merit. **OQMD** gives an independent hull distance, **OpenAlex** a compound's works and their thin-film subset, **PubChem** GHS statements where a record exists. A versioned **element hazard table** in the repo scores toxicity because it is complete; PubChem is caveat evidence because it is not.

## 4. Domain handling a materials scientist checks first

**DFT gaps are underestimated.** Correction is a config strategy, the functional is shown beside every value, a corrected value is labelled, and the gate applies to the effective gap. **Bulk stability is not thin-film processability.** Hull distance says nothing about deposition routes or lattice match; the tool does not model them, the model may not speculate about them, and every output says so.
...
The missing-data policy is the interesting part, because the first version was wrong. Renormalising over the criteria that have data put five candidates with **no** dielectric value above HfO2 on the fixture: an unknown criterion cannot pull a score down, so a candidate that would have scored poorly on it is helped by the gap. The known-answer check caught it. The shipped default is `no_credit`: an unknown criterion earns nothing, is shown as unknown and lowers confidence, so less data can never mean a higher rank. Scores closer than a tie band print as one tier with the order inside it declared arbitrary.

## 6. Retargeting to a new material class

The seam is one criterion, not a rewrite. Exactly these files change to instantiate a class:
...
Nothing in `scoring/`, the refutation pass, the guard, the templates or the front end names a class. An FDE fills those files with the site's scientists: agree the figure of merit and its saturation points, name the workhorses the self-check must reproduce (it fails loudly if a known material vanishes or sinks, and names the honest reasons when it does), choose the substrate, prune the cation universe, switch on the refutation rules the class needs; a class needs a few illustrative fixture records so its demo runs offline. Two limits stay fixed: the universe is oxides (the anion is a second seam, untouched here, which is why fluoride UV windows were not the second class), and the arithmetic and the model boundary are not per-class.

## 7. Refutation pass

A shortlist entry is a **conjecture**, held provisionally and stated so that it can be falsified. After ranking, a stage whose only job is to argue against each shortlisted candidate adds its caveats: hygroscopicity, absent figure-of-merit data, contested stability, metastability, corrected or near-threshold gaps, hazard tiers, thin literature, incomplete retrieval, and, where a profile switches them on, a competing polymorph within a few meV/atom and a 4f-element gap DFT cannot be trusted with. An optional model pass elaborates over the same facts, delimited as data, under the guards of section 2. The tool proposes; the bench disposes.

## 8. Constraint handling

Requests are sorted into bins by rules **before any model sees them**. *Bin 1, architecturally impossible* (wet-lab triggers, private data, paywalled sources): the capability does not exist, and there are no stub tools to decline politely. *Bin 2, configuration deviation*: "include lead-containing compounds" looks like a safety bypass but a Pb-ferroelectrics group has a real reason; it is permitted, printed at the top of the output, caveated, and logged. *Bin 3, evidence-integrity attacks* ("cite a paper supporting this"): refused, naming what would have been fabricated. *Bin 0, override attempts*: nothing to refuse, since there is no mode in which the constraints are lifted; the run proceeds as the plain request would and the attempt is logged. Retrieved text reaches a model only JSON-encoded inside delimited blocks; model output is validated against a schema and a digits-only numeric guard, and a value in words passing it is the known residual gap.

## 9. Agentic where, and how it is deployed

The system decides at the boundary (refuse, proceed, proceed with a logged deviation); asks before acting when a request changes something material; carries a conversation about a result without re-deriving it; acquires data on demand from an allowlist of read-only routes, reporting gaps with no public route as unfillable; and re-checks itself against ground truth after every change to its cache. It is not agentic where numbers are concerned. The web app assistant and the MCP server are clients for the same tools. One YAML with named profiles and a site overrides file the admin panel writes; Docker compose runs offline once the cache is warmed, and a local 8B model loses nothing because the model's job is small. Every tagged release warms, self-checks and packages a cache, then re-runs the PI's request with the network disabled.

### Feedback to the platform team

Two aggregates serve the platform team rather than the scientist. The deviation log answers which gates sites relax and who decided: repeated workarounds in requests mean the profile wants retuning for that group; a site override means the shipped default is wrong for that class of site. The gap ledger, one record per run, answers which properties are absent from every public source versus merely unfetched by this cache: the first is a data-integration or measurement decision, the second an ops problem. Both are read-only: they inform a human who changes config. Nothing tunes a default, or the ranking stops being a function of cache plus config.
